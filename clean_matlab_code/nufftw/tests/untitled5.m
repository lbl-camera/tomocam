...
% analytic image domain deapodization - doesn't include replication and
% sampling correction
x1 = (-ceil(G/2):floor(G/2-1));
%x1 = (-ceil(N/2):floor(N/2-1));
d2 = sinc(sqrt((pi*W*x1/G).^2-beta^2)/pi);
d2 = d2.*(-1).^(1:G).*sinc(x1/SG).^2;
%[d11,d12] = meshgrid(d2,d2);
d2 = d2'*d2;
d2 = d2(nufft_st.nstart+(1:nufft_st.N),nufft_st.nstart+(1:nufft_st.N));
im2 = imno./d2;

% analytic image domain deapodization, including replication
% not including sampling correction
d3 = sinc(sqrt((pi*W*x1/G).^2-beta^2)/pi) + sinc(sqrt((pi*W*(x1+G)/G).^2-beta^2)/pi) + sinc(sqrt((pi*W*(x1-G)/G).^2-beta^2)/pi);
d3 = d3.*(-1).^(1:G).*sinc(x1/SG).^2;
%[d11,d12] = meshgrid(d3,d3);
d3 = d3'*d3;
d3 = d3(nufft_st.nstart+(1:nufft_st.N),nufft_st.nstart+(1:nufft_st.N));
im3 = imno./d3;

% beatty deapodization
SG = ceil(S*G);
nkb = floor(length(KB)/2);
kb = zeros(1,SG);
kb(1:10) = KB(11:end);
kb(end-10+1:end) = KB(1:10);
kb = ifftshift(kb);
d4 = ifftshift(ifft(kb));
ns = ceil((SG-G)/2);
d4 = d4(ns+(1:G));
d4 = d4.*sinc(x1/SG).^2;
d4 = d4'*d4;
d4 = d4(nufft_st.nstart+(1:nufft_st.N),nufft_st.nstart+(1:nufft_st.N));
im4 = imno ./ d4;
...
figure
imshowz([get_image_errors(imagedirect,image_deap) get_image_errors(imagedirect,im1) get_image_errors(imagedirect,im2) get_image_errors(imagedirect,im3)])
figure
imshowz(get_image_errors(imagedirect,im4))
