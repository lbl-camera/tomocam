    ...
    return ret*ret;
                ...
                struct DIR
        {
            ...
            template <typename Tuple>
                    __host__ __device__
                    thrust::tuple<OUT,OUT> operator()(Tuple x)
            {
                thrust::tuple<OUT,OUT> out;
                IN G = thrust::get<0>(x);
                IN dG = thrust::get<1>(x);
                float F = thrust::get<2>(x);
                IN Gt=G+tau*dG;
//    OUT Gtratio=rsqrt(norm(Gt));;//1/abs(Gt);//abs(G+tau*dG);
                OUT Gtratio=1/abs(Gt);//abs(G+tau*dG);
                OUT Fratio=abs(F)*Gtratio;
                //OUT Gtratio=1/abs(Gt);//abs(G+tau*dG);
                //    OUT derr = (dG*conj(G) * (1-F/abs(Gt))).real();
                OUT derr = (conj(dG)*Gt * (1-Fratio)).real();
                
                //OUT ph = sin(arg(Gt*conj(dG)));
                //ph*=ph;
                //OUT derr = (conj(dG)*Gt * (1-F*Gtratio)).real();
                //OUT d2err = (dG*conj(dG) * (1-F*Gtratio*ph)).real();
                //Gtratio*=Fratio*Gtratio*Gtratio;
                OUT tmp=(Gt.imag() *dG.real()-Gt.real()*dG.imag())*Gtratio;
                tmp*=tmp;
                OUT d2err=norm(dG)-Fratio*tmp;

                
                thrust::get<0>(out) = derr;
                thrust::get<1>(out) = d2err;
                return out;
            }
        };
