...
//    loop through grid
        ...
        if( 0){
        if(tid == 0){
	     sum_points<<<1,blockDim>>>(binned_points_x,binned_points_y,point_value,grid_size, binned_points,ppb,idx,x,y, kb_table_size,kb_table_scale, grid_value);
	     __syncthreads();
