...
// kbwaits float2 float2
// kbwaits float, float,float,float
...
//    loop through grid
	...
	//	point_pos=make_float2(binned_points_x[idx+j],binned_points_y[idx+j]);
	...
	}}else  {                 // large dimensions

     __shared__ cusp::complex<float> point_value_cache[SHARED_SIZE];   
    __shared__ float2 point_pos_cache[SHARED_SIZE];
   ...
   // Lets try to load all points to shared memory /
        for(int j = tid;j<ppb;j+= blockDim.x){
	       ...
	       point_pos_cache[j]=make_float2(binned_points_x[idx+j],binned_points_y[idx+j]);
        }
        __syncthreads();
  ...
  int bblock;
    ...
    __shared__ cusp::complex<float> sum_t[BLOCKSIZE];
	  ...
	  // const int point = binned_points[idx+j];
        ...
        __syncthreads();
        int b = bblock;// each b threads takes care of a pixel
	    ...
	    //   cusp::complex<float> my_sum = 0;
            ...
            for(int j = (tid&(b-1));j<ppb;j+=b){
                 float w=kb_weight(make_float2(x,y),point_pos_cache[j],
			       kb_table_size,kb_table_scale, kb_table);
...
//--------------------------------
 ...
 //textures: filter lut, data value
...
// do the calculation           
 grid_points_cuda_mex_interleaved_kernel<<<grid,block_size>>>(
...
grid_points_cuda_interleaved_mex(d_samples_values, npoints, 
				    grid_size, d_samples_per_bin, d_bin_dimensions_x, d_bin_dimensions_y,  d_samples_in_bin, d_bin_start_offset, d_bin_location,    d_bin_points_x, d_bin_points_y,  nbins, d_kernel_lookup_table,kernel_lookup_table_size,  kernel_lookup_table_scale,  d_grid_values);
