...
__global__ void grid_points_cuda_mex_interleaved_kernel(const float * point_x,
  ...
  __shared__ cusp::complex<float> value;
    ...
    if(bin_dimension_x[i]*bin_dimension_y[i] < 64 || points_per_bin[i] > SHARED_SIZE){
      ...
      if(1){
        if(tid == 0){
            sum_points<<<1,BLOCKSIZE>>>( point_x, point_y,
...
if(1){
	  ...
	  //	  sum_points_mid<<<ppb/bd+1,bd>>>(binned_points_x,binned_points_y,point_value,binned_points,ppb,idx,dims,corner,grid_size,  kb_table_size,kb_table_scale,texRef,grid_value);
	    //     __syncthreads();
     //	     cudaDeviceSynchronize();
	     //     __syncthreads();
