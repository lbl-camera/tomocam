...
%   bNorm = norm(abs(b),1);
   ...
   [bs,idx] = sort(abs(b),'descend'); % Descending.

   if true
    csbh=cumsum(b)-tau;
    alphai=csbh./(1:numel(b))';
    itn=find(alphai>=b,1,'first');
    alpha=alphai(itn-1);
%   x(idx) = max(0, b - alpha.*sign(b));
     x = b.* max(0, abs(b) - alpha);
  
%   x = subsasgn(x, '()',max(0,b - alpha))
   else
