...
% Dt=round(360/nangles); %spacing in degrees
% [tt,qq]=meshgrid(0:Dt:360-Dt,(1:(Ns))-floor((Ns+1)/2)-1);
...
fprintf(sprintf('time=(gridding:%g, spmv:%g)\n',t1,t2));
...
tic;
for ii=1:ntries;
    Gradon_spmv=gnuradon_spmv(Gxy);
end
tradon_spmv=toc/ntries*1e3; %msec
...
tic;
for ii=1:ntries;
    GG_spmv=gnuiradon_spmv(Gradon_spmv);
end
tiradon_spmv=toc/ntries*1e3; %msec
...
%%
 tic;for ii=1:ntries;    GG=gnuiradon(Gradon);end;tiradon=toc/ntries*1e3
 imagesc(abs(qxyXqt(fftshift(fft(fftshift(Gradon,1)),1))).^.2)
colormap hot
