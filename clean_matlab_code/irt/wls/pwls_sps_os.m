 function [xs, info] = pwls_sps_os(x, yi, wi, Gb, R, niter, pixmax, ...
		denom, gi, relax0, chat)
%function [xs, info] = pwls_sps_os(x, yi, wi, Gb, R, niter, pixmax, ...
%		denom, gi, relax0, chat)
%
...
%	Gb	[nd,np]		Gblock object, aij >= 0 required!
%					or sparse matrix (implies nsubset=1)
%	R			penalty object (see Robject.m)
%	niter			# of iterations
%	pixmax	[1] or [2]	max pixel value, or [min max] (default [0 inf])
%	denom	[np,1]		precomputed denominator (optional)
%	gi	[nb,na]		precomputed row sums of |Gb| (optional)
...
if ~isvar('denom') | isempty(denom)
	denom = Gb' * col(gi .* wi);	% requires real g_ij and g_ij >= 0
end, clear gi
...
for iter = 2:niter
	...
	for iset=1:nblock
		...
		grad = Gb{iblock}' * resid(:); % G' * W * (y - G*x)
