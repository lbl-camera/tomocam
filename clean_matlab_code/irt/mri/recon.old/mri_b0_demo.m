...
%	using conjugate gradient (CG) algorithm to minimize a
%	penalized least-squares cost function.
%	Matrix vector multiplications are performed rapidly
%	using a combination of time segmentation and NUFFT.
%	The temporal interpolator is min-max optimal.
%
%	This demo compares "slow, exact" versus "fast, approximate" 
%	iterative MR reconstruction.  The slow part is indeed slow.
%
%	To run, this requires the mex file mvmult.mex... which one can
%	create using 'mex mvmult.c' at command prompt.
%	The linux version mvmult.mexglx is already compiled and available.
%
%	Brad Sutton, University of Michigan
%	June 2002
%
%  Code contained in this will appear in 2 articles:
%     B.P. Sutton and J.A. Fessler and D.C. Noll.  Fast, iterative, 
%            field-corrected image reconstruction for MRI.  Submitted 
%            IEEE Trans. Med. Im., June 2002
%
%     J.A. Fessler and B.P. Sutton.  Nonuniform fast fourier transforms using
%            min-max temporal interpolation.  Accepted IEEE Trans. Signal
%            Processing, 2002
%
%
%SIMULATION VERSION
...
%Set up mask
  ...
  sprintf('Forming data from 256x256 object. \n  May take time please be patient')
...
if 1
    A_fast = fast_mr(kx,ky,FOV,N,2*N,5,tt_ext,we(:),1,5,1);
