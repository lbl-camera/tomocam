...
%|	'fast'	0|1		1 to use new faster (?) method
%|
%| out
%|	phantom		[nx ny]	gray-scale image
%|
...
profile on
x0 = ellipse_im(ig, [], 'oversample', 3, 'fast', 1);
profile off
profile report
...
for fast = 1:-1:0
	cpu etic
	mat = ellipse_im(ig, ell, 'oversample', over, 'fast', fast);
	if fast
		cpu etoc 'fast time:'
		area.fast = sum(mat(:)) * abs(ig.dx * ig.dy);
	else
		cpu etoc 'slow time:'
		area.slow = sum(mat(:)) * abs(ig.dx * ig.dy);
	end

	im(2+fast, mat, sprintf('mat, fast=%d', fast)), cbar
	im(5+fast, (mat-asp)*over^2, 'difference: (mat-asp)*over^2'), cbar
