...
%|	mask	[nx,ny[,nz]]	logical support
%| option
%|	'x'	[nx,ny[,nz]]	optional input test image
...
% todo: need user option to replace jf_equal by equivs
% for objects that use 'parfor' where A*x can vary slightly between runs
jf_equal(y1, y2)
...
% A()
% for single and multiple arguments and test consistency thereof
c1 = A(:,1);
c2 = A(:,2);
c12 = A(:, [1 2]);
jf_equal(c12, [c1 c2])
