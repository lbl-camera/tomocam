% source code to produce analytical predictions for 
% 2d fast variance predictions paper for IEEE.
%
...
if streq(t.fov, 'ellipse')
    if 1 % d_0(\phi) at image center
        fov = x_mask .* y_mask ./ sqrt((x_mask .* cos(phiw)).^2 + (y_mask .* sin(phiw)).^2);
        fov = 2 .* fov(:) .* dx;
    end
