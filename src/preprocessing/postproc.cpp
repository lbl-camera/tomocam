...
namespace tomocam::preprocessing {
    ...
    void postproc_(Partition<T> soln, Partition<T> soln2, int npad, int device) {
        ...
        int nparts = config.num_of_partitions(soln.dims(), soln.bytes());
        ...
        while (scheduler.has_work()) {
            ...
            if (work.has_value()) {
                ...
                // remove padding
                auto d_soln2 = gpu::unpad2d(d_soln, 2 * npad, PadType::SYMMETRIC);
    ...
    DArray<T> postproc(DArray<T> &soln, int npixels) {
        ...
        int ndevices = config.num_of_gpus();
