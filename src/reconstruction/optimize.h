      ...
      public:
        ...
        Array<T> run(Array<T> sol, int max_iters, T step_size, T tol) {
            ...
            for (int iter = 0; iter < max_iters; iter++) {
                ...
                // update theta
                T temp = 0.5 * (std::sqrt(std::pow(t, 4) + 4 * std::pow(t, 2)) -
                                std::pow(t, 2));
                t = tnew;
                tnew = temp;
        ...
        Array<T> run2(Array<T> &sol, int max_iters, T step_size, T tol, T xtol) {
            ...
            for (int iter = 0; iter < max_iters; iter++) {
                while (true) {
                    ...
                    T fx = error_(sol);
                    T fy = error_(y);
                    T gy_sq = dot<T>(g, g); // NOT_ON_GPU
#ifdef MULTIPROC
                    fx = multiproc::mp.SumReduce(fx);
                    fy = multiproc::mp.SumReduce(fy);
                    gy_sq = multiproc::mp.SumReduce(gy_sq);
#endif
                    T gy = 0.5 * step_size * std::sqrt(gy_sq);
                    if (fx > (fy + gy))
                        step_size *= 0.9;
                    else {
                        ...
                        xerr = array::norm2(sol - x); // NOT_ON_GPU
#ifdef MULTIPROC
