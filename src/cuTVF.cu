...
namespace tomocam {
    ...
    __global__ void tvf_update_kernel(dev_arrayf model, dev_arrayf objfn, float p, float sigma, float lambda) {
            ...
            for (int ix = 0; ix < 3; ix++)
                for (int iy = 0; iy < 3; iy++)
                    for (int iz = 0; iz < 3; iz++)
                        temp += weight(ix, iy, iz) * pot_func(v - s_val[i + ix][j + iy][k + iz], p, sigma);
            objfn(x, y, z) += lambda * temp;
        }
    }

    void add_tv_func(dev_arrayf &model, dev_arrayf &objfn, float p, float sigma, float lambda, cudaStream_t stream) {

        // CUDA kernel parameters
        Grid grid(objfn.dims());

        // update gradients of objective function inplace
        tvf_update_kernel<<< grid.blocks(), grid.threads(), 0, stream >>>(model, objfn, p, sigma, lambda);
    }
