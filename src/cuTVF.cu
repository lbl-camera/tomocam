...
namespace tomocam {
    ...
    __global__ void tvf_update_kernel(DeviceMemoryf model, DeviceMemoryf objfn, float p, float sigma) {
    ...
    void add_tvf(DeviceArrayf &model, DeviceArrayf &objfn, float p, float sigma, cudaStream_t stream) {

        // CUDA kernel parameters
        Grid grid(objfn.dims());

        // update gradients of objective function inplace
        tvf_update_kernel<<< grid.blocks(), grid.threads(), 0, stream >>>(model, objfn, p, sigma);
    }
