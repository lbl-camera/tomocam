...
namespace tomocam {
    ...
    void inverseRadon(Partition<float> sino, Partition<float> output, float center, float over_sample,
    ...
    void iradon(DArray<float> &input, DArray<float> &output, float * angles,
        ...
        for (int i = 0; i < p1.size(); i++) {
            ...
            threads.push_back(std::thread(inverseRadon, p1[i], p2[i], center, over_sample, dAngles[i], kernels[i], device));
        }
