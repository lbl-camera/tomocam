...
namespace tomocam {
    ...
    template <typename T>
    T gradient2_(Partition<T> f, Partition<T> sinoT, Partition<T> df,
        ...
        while (s.has_work()) {
            ...
            if (work.has_value()) {
                ...
                auto [d_g, pf] = gradient2<T>(d_f, d_sinoT, psf, work_s);
    ...
    // Multi-GPU calll
    template <typename T>
    std::tuple<DArray<T>, T> gradient2(DArray<T> &solution, DArray<T> &sinoT,
        ...
        for (int i = 0; i < nDevice; i++) {
            pfunc[i] = gradient2_<T>(p1[i], p2[i], p3[i], psfs[i], i);
        }
    ...
    template std::tuple<DArray<float>, float> gradient2<float>(DArray<float> &solution,
        DArray<float> &sinoT, const std::vector<PointSpreadFunction<float>> &psfs);
    // specialization for double
    template std::tuple<DArray<double>, double> gradient2<double>(DArray<double> &solution,
        DArray<double> &sinoT, const std::vector<PointSpreadFunction<double>> &psfs);
