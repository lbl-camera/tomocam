...
namespace tomocam {
    ...
    void tv_func_(Partition<float> model, Partition<float> objfn, float p, float sigma, int device) {
        ...
        for (int i = 0; i < n_batch; i++) {
            ...
            for (int j = 0; j < n_sub; j++)
                add_tvf(d_model[j], d_objfn[j], p, sigma, streams[j]);
    ...
    // multi-GPU call
    void eval_tvfunc(DArray<float> &model, DArray<float> &objfn, float p, float sigma) {
