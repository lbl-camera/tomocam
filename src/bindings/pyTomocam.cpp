...
np_array_t<float> backproject_wrapper(np_array_t<float> &sino,
    ...
    auto sino2 = tomocam::preprocessing::preproc(arg1, cen);

    // backproject call
    auto tmp = tomocam::backproject(sino2, getVec<float>(angs));

    // undo the padding
    auto bproj = tomocam::preprocessing::postproc(tmp, nrays);
...
np_array_t<float> mbir_wrapper(np_array_t<float> &np_sino,
    ...
    tomocam::DArray<float> recon = tomocam::reconstruction::mbir2(
        x0, sino, angles, center, num_iters, sigma, tol, xtol);
...
PYBIND11_MODULE(cTomocam, m) {
    ...
    // set gpu paramters
    m.def("set_num_of_gpus", [](int num) { config.num_of_gpus(num); });
