...
namespace tomocam {
    ...
    void project_(Partition<T> input, Partition<T> sino,
        ...
        auto nugrid = Grid(nproj, ncols, angles.data(), device);

        // create subpartitions
        int nparts = config.num_of_partitions(input.dims(), input.bytes());
    ...
    DArray<T> project(DArray<T> &input, const std::vector<T> &angles) {
        ...
        for (int i = 0; i < nDevice; i++) {
            threads[i] =
                std::thread(project_<T>, p1[i], p2[i], std::cref(angles), i);
        }
