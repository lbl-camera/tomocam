...
namespace tomocam {
    namespace gpu {
         ...
         **************************/
        template <typename T>
        __global__ void axpy_kernel(T *x, T a, const T *y, int n) {
            int idx = Index1D();
            if (idx >= n) return;
            x[idx] = a * x[idx] + y[idx];
        }
        ...
        void axpy(T *x, T a, const T *y, int n) {
            Grid grid(n);
            axpy_kernel<<<grid.blocks(), grid.threads()>>>(x, a, y, n);
        }
        template void axpy<float>(float *, float, const float *, int);
        template void axpy<double>(double *, double, const double *, int);

        /**************************
         * x +=   a * y
         **************************/
        template <typename T>
        __global__ void xpay_kernel(T *x, T a, const T *y, int n) {
            int idx = Index1D();
            if (idx >= n) return;
            x[idx] += a * y[idx];
        }
        ...
        void xpay(T *x, T a, const T *y, int n) {
            Grid grid(n);
            xpay_kernel<<<grid.blocks(), grid.threads()>>>(x, a, y, n);
        }
        template void xpay<float>(float *, float, const float *, int);
        template void xpay<double>(double *, double, const double *, int);
         ...
         **************************/
        template <typename T>
        __global__ void gpu_divide_arrays(const T *a, const T *b, T *result,
                                          int size) {
            int idx = Index1D();
            if (idx < size) result[idx] = a[idx] / b[idx];
        }
        ...
        template void init_array(gpu::complex_t<double> *, gpu::complex_t<double>,
                                 int);
         ...
         ***************************/
        template <typename T>
        __global__ void gpu_dot(const T *a, const T *b, T *c, int size) {
        ...
        T dot(const T *a, const T *b, int size) {
            ...
            gpu_dot<<<grid.blocks(), grid.threads(), shamem>>>(a, b, d_result, size);
            SAFE_CALL(
        ...
        // specialize
        template void cast_array_to_complex(const float *, gpu::complex_t<float> *,
                                            int);
        ...
        template void cast_array_to_real(const gpu::complex_t<float> *, float *,
                                         int);
        template void cast_array_to_real(const gpu::complex_t<double> *, double *,
                                         int);
