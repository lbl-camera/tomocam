...
namespace tomocam::utils {
    ...
    inline void write_h5(const DeviceArray<T> &arr) {
        Writer writer("debug.h5");

        // copy data to DAarray
        DArray<T> data(arr.dims());
        SAFE_CALL(cudaMemcpy(data.begin(), arr.dev_ptr(), arr.size() * sizeof(T),
    ...
    void printf(const DeviceArray<Real> &d) {
        ...
        for (int i = 0; i < nrows; i++) {
            for (int j = 0; j < h.ncols(); j++) { std::cout << h(0, i, j) << " "; }
            std::cout << std::endl;
        }
    }
    ...
    void print(const DeviceArray<Complex_t> &d) {
        ...
        for (int i = 0; i < nrows; i++) {
            for (int j = 0; j < h.ncols(); j++) {
                std::cout << "(" << h(0, i, j).real() << ", " << h(0, i, j).imag()
                          << ") ";
            }
            std::cout << std::endl;
        }
    ...
    namespace log {
        inline std::string dims2str(const dim3_t &dims) {
            return ("size: " + std::to_string(dims.x) + "x" +
