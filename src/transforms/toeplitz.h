...
using tomocam::to_real;
using tomocam::transforms::NUFFT::Grid;
using tomocam::transforms::NUFFT::nufft2d1;
...
namespace tomocam::transforms {
    ...
    class PointSpreadFunction {
      ...
      public:
        ...
        PointSpreadFunction(const Grid<T> &grid)
            : batch_size_(0), r2c_(0), c2r_(0), initialized_(false) {
            ...
            auto temp = nufft2d1(ones, grid, out_dims);

            // get the real part
            auto psf = to_real(temp);
        ...
        // move constructor
        PointSpreadFunction(PointSpreadFunction &&other) noexcept
            : batch_size_(other.batch_size_), initialized_(other.initialized_),
              r2c_(other.r2c_), c2r_(other.c2r_), psf_(std::move(other.psf_)),
              xpad_(std::move(other.xpad_)) {
            other.batch_size_ = 0;
        ...
        PointSpreadFunction &operator=(PointSpreadFunction &&other) noexcept {
            if (this != &other) {
                // destroy existing plans
                if (initialized_) {
                    SAFE_CALL(cufftDestroy(r2c_));
                    SAFE_CALL(cufftDestroy(c2r_));
                }
