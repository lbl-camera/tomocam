...
#define NUFFT_CALL(ans)                                                             \
    { nufft_check((ans), __FILE__, __LINE__); }
...
using tomocam::DeviceArraycd;
using tomocam::DeviceArraycf;
...
namespace tomocam::transforms {
    namespace NUFFT {
        ...
        inline void nufftPlan(dim3_t dims, int type, int iflag, int device_id,
            ...
            NUFFT_CALL(cufinufftf_makeplan(type, ndim, nmodes, iflag, ntransf, tol,
                                           &plan, &opts));
        }
        ...
        inline void nufftPlan(dim3_t dims, int type, int iflag, int device_id,
            ...
            cufinufft_default_opts(&opts);
            opts.gpu_device_id = device_id;
            NUFFT_CALL(cufinufft_makeplan(type, ndim, nmodes, iflag, ntransf, tol,
                                          &plan, &opts));
        }

        // 2-dimensional NUFFT from non-uniform to uniform grid (single precision)
        inline DeviceArraycf nufft2d1(DeviceArraycf &c, const Grid<float> &nugrid,
                                      dim3_t dims = {0, 0, 0}) {
            ...
            NUFFT_CALL(cufinufftf_setpts(plan, nugrid.size(), nugrid.x(), nugrid.y(),
        ...
        // precision)
        inline DeviceArraycd nufft2d1(DeviceArraycd &c, const Grid<double> &nugrid,
                                      dim3_t dims = {0, 0, 0}) {
            ...
            // set the non-uniform points
            NUFFT_CALL(cufinufft_setpts(plan, nugrid.size(), nugrid.x(), nugrid.y(),
                                        nullptr, 0, nullptr, nullptr, nullptr));
        ...
        inline DeviceArraycf nufft2d2(DeviceArraycf &fk, const Grid<float> &nugrid) {
