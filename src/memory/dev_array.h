...
namespace tomocam {
    ...
    class DeviceArray {
      ...
      public:
        ...
        DeviceArray(dim3_t d) : dims_(d) {
            halo_ = {0, 0};
            size_ = d.x * d.y * d.z;
            dev_ptr_ = make_cuniquePtr<T>(size_);
        }
        ...
        DeviceArray(const Partition<T> &rhs)
            : dims_(rhs.dims()), halo_(make_int2(rhs.halo()[0], rhs.halo()[1])),
              size_(rhs.size()), dev_ptr_(make_cuniquePtr<T>(rhs.size())) {
            H2D(dev_ptr_.get(), rhs.begin(), rhs.bytes());
        }

        // cuniquePtr makes destructor redundant
        ~DeviceArray() = default;

        // needed for scheduler and multi-threading
        DeviceArray(const DeviceArray<T> &rhs) {
            dims_ = rhs.dims_;
            size_ = rhs.size_;
            halo_ = rhs.halo_;
            dev_ptr_ = make_cuniquePtr<T>(size_);
            SAFE_CALL(cudaMemcpy(dev_ptr_.get(), rhs.dev_ptr_.get(), bytes(),
        ...
        // get halo
        [[nodiscard]] int2 halo() const { return halo_; }
        ...
        void copy_to(Partition<T> &rhs) const {
            if (dims_ == rhs.dims()) {
                D2H(rhs.begin(), dev_ptr_.get(), rhs.bytes());
            } else {
        ...
        std::vector<T> copy_to_host() const {
            ...
            D2H(h_ptr.data(), dev_ptr_.get(), bytes());
            return h_ptr;
        }
        ...
        DeviceArray<T> operator-(const DeviceArray<T> &arr) const {
            ...
            if (dims_ == arr.dims_)
                gpu::subtract_arrays<T>(dev_ptr_.get(), arr.dev_ptr_.get(),
                                        res.dev_ptr_.get(), size_);
            else { throw std::runtime_error("Array dimensions do not match in -"); }
            return res;
        }
    ...
    template <typename T>
    DeviceArray<T> to_real(const DeviceArray<gpu::complex_t<T>> &arr) {
    ...
    template <typename T>
    DeviceArray<gpu::complex_t<T>> to_complex(const DeviceArray<T> &arr) {
