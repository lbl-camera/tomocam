...
__global__ void init_polargrid(complex_t * point_pos, int cen,
        int num_lines, int num_angs, float d_th){
    int i = threadIdx.x + blockDim.x * blockIdx.x;
    int j = threadIdx.y + blockIdx.y * blockIdx.y;
    if ((i < num_lines) && (j < num_angs)){
        float th = j * d_th;
        int idx = i * num_angs + j;
        point_pos[idx].x = (i - cen) * cos(th) + cen;
        point_pos[idx].y = (i - cen) * sin(th) + cen;
    }
}
