...
int main(int argc, char **argv) {
    ...
    tomocam::DArray<double> f(dim2);
    for (int i = 0; i < f.size(); i++) f[i] = random<double>();

    // create device array
    tomocam::DeviceArray<double> d_f(dim2);
    cudaMemcpy(d_f.dev_ptr(), f.begin(), f.bytes(), cudaMemcpyHostToDevice);

    // angles 0 - 180
    std::vector<double> theta(nproj);
    ...
    // copy data to host
    tomocam::DArray<double> g(df.dims());
    ...
    tomocam::DeviceArray<tomocam::gpu::complex_t<double>> d_c(dim1);
    tomocam::DeviceArray<tomocam::gpu::complex_t<double>> df2(dim2);
    tomocam::NUFFT::nufft2d2(d_c, dfc, nugrid);
    tomocam::NUFFT::nufft2d1(d_c, df2, nugrid);
    auto dg2 = tomocam::real(df2, cudaStreamPerThread);

    tomocam::DArray<double> g2(df2.dims());
    // copy data to host
